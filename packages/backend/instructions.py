...
# Every merged file opens with this line so that its deployed copies
# (`~/.claude/output-styles/duet-executor.md`, `~/.claude/agents/duet-*.md`,
# Codex / Antigravity / Kimi prompt files) say where they come from. An agent
# that finds the file on disk must edit the named source, not the copy. Same
# marker substring as `services.deploy_instructions.GENERATED_BANNER`.
GENERATED_BANNER = "AUTO-GENERATED by Duet"
PLATFORM_SOURCE_REF = "@Duet.git/packages/instructions"


def _banner(sources: str) -> str:
    return f"<!-- {GENERATED_BANNER} from {PLATFORM_SOURCE_REF}/{sources} — edit the source, not this file -->\n\n"
...
def _merge_one_agent(
    ...
    return _banner(f"bootstrapper.md + {agent_rel_path}") + merged, None
...
def _build_bare_session_prompt(bootstrapper_text: str) -> str:
    ...
    return _banner("bootstrapper.md") + bare.rstrip() + "\n"
